...
const Form = props => {
    ...
    props.json.elements.map((e, index) => {
        ...
        else if (e.type === 'dropdown') form.push(<DropdownElement key={index} onChange={onChange} index={index} title={e.name} items={e.items} />);
        else if (e.type === 'radio') form.push(<RadioElement key={index} onChange={onChange} index={index} title={e.name} items={e.items} />);
        else if (e.type === 'range') form.push(<RangeElement key={index} onChange={onChange} index={index} title={e.name} min={e.min} max={e.max} />);
        else if (e.type === 'imagepicker') form.push(<ImagePickerElement key={index} onChange={onChange} index={index} title={e.name} items={e.items} />);
        else if (e.type === 'datepicker') form.push(<DateTimePickerElement key={index} onChange={onChange} index={index} title={e.name} mode={'date'}/>);
        else if (e.type === 'timepicker') form.push(<DateTimePickerElement key={index} onChange={onChange} index={index} title={e.name} mode={'time'}/>);
